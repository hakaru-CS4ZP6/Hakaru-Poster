...
\footnotesize{In the case of unreachable distributions, we have implemented models in terms of their PDF/PMF.}
...
\footnotesize{Hakaru's validity can be tested by checking if it recognizes known relationships between distributions. More specifically:

\bigskip


\begin{addmargin}[4em]{4em}
\textit{Hypothesis: By applying appropriate transformations to implementations of distributions, A and B, we can create two distinct Hakaru programs which, when passed to hk-maple, will reduce to equivalent Hakaru code.}
\end{addmargin}
