...
\begin{itemize}
	\item[\textbf{$\star$}] \textit{A Hakaru program is a probabalistic model.}
	\item[\textbf{$\star$}] \textit{A Hakaru program is an implementation of a statistical distribution.}
	\item[\textbf{$\star$}] \textit{Values that are pulled from a distribution are called samples, or measures.}
	\item[\textbf{$\star$}] \textit{In mathematics, we write} $\textit{X} \sim Normal(0, 1)$ \textit{to denote that the random variable is normally distributed, while in Hakaru code we write} {\tt \tiny{x <$\sim$ normal(0, 1)}} \textit{, which pulls a sample from the distribution and binds it to the variable} {\tt \tiny{x.}}
